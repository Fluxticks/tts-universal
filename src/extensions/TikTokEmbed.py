...
def embed_from_post(post_info: TikTokVideo | TikTokSlide) -> Embed:
    ...
    embed = Embed(
        color=0xFF0050, title=title, description=description, url=post_info.url
    )

    embed.set_author(
        name=f"{post_info.author_display_name}",
        url=post_info.author_url,
        icon_url=post_info.author_avatar,
    )
    embed.set_footer(
    ...
    embed.add_field(
        inline=True, name="View Count 👀", value=f"{post_info.view_count:,}"
    )
    embed.add_field(inline=True, name="Like Count ❤️", value=f"{post_info.like_count:,}")
    embed.add_field(
        inline=True, name="Comment Count 💬", value=f"{post_info.comment_count:,}"
    )
    embed.add_field(
        inline=True, name="Share Count 🚀", value=f"{post_info.share_count:,}"
    )
...
def embed_from_slide(slide_info: TikTokSlide) -> list[Embed]:
    ...
    primary_embed.insert_field_at(
        index=0, inline=False, name="Image Count 📸", value=len(slide_info.images)
    )
...
async def respond_to_message(
    message: Message, post_data: TikTokVideo | TikTokSlide
) -> bool | None:
...
async def respond_to_interaction(
    interaction: Interaction, post_data: TikTokVideo | TikTokSlide
):
    if isinstance(post_data, TikTokVideo):
        if os.path.getsize(post_data.file_path) >= MAX_FILE_BYTES:
            await interaction.edit_original_response(
                content=COG_STRINGS["tiktok_file_too_big"]
            )
        ...
        embed = embed_from_video(post_data)
        await interaction.edit_original_response(
            content="", embed=embed, attachments=[File(f"{file}")]
        )
        os.remove(file)
    else:
        embeds = embed_from_slide(post_data)
        files = [File(x, filename=x) for x in post_data.images]
        await interaction.edit_original_response(
            content="", embeds=embeds, attachments=files
        )
        for file in post_data.images:
            os.remove(file)
...
class TikTokEmbedAdmin(GroupCog, name=COG_STRINGS["tiktok_admin_group_name"]):
    ...
    @command(
        name=COG_STRINGS["tiktok_enable_messages_name"],
        description=COG_STRINGS["tiktok_enable_messages_description"],
    )
    ...
    @command(
        name=COG_STRINGS["tiktok_disable_messages_name"],
        description=COG_STRINGS["tiktok_disable_messages_description"],
    )
...
class TikTokEmbed(GroupCog, name=COG_STRINGS["tiktok_group_name"]):
    ...
    @command(
        name=COG_STRINGS["tiktok_embed_name"],
        description=COG_STRINGS["tiktok_embed_description"],
    )
    ...
    async def embed(self, interaction: Interaction, url: str):
        ...
        if not found_urls:
            await respond_or_followup(
                message=COG_STRINGS["tiktok_warn_invalid_url"], interaction=interaction
            )
            return
        ...
        try:
            ...
            await respond_or_followup(
                COG_STRINGS["tiktok_warn_captcha_failed"],
                interaction=interaction,
                delete_after=30,
            )
        except DownloadFailedException:
            await respond_or_followup(
                COG_STRINGS["tiktok_warn_download_failed"],
                interaction=interaction,
                delete_after=30,
            )
        except ResponseParseException:
            await respond_or_followup(
                COG_STRINGS["tiktok_warn_parse_error"], interaction, delete_after=30
            )
