...
# AUTHOR_ID = 202978567741505536
global MUSIC_AUTHOR
MUSIC_AUTHOR = "fuxticks#1809"
...
class SongRequest:
    ...
    async def get_song(self) -> Union[list["SongRequest"], "SongRequest", None]:
        ...
        match self.request_type:
            ...
            case SongRequestType.YOUTUBE_PLAYLIST:
                ...
                song_requests = parse_playlist_response(
                    self.raw_request, self.request_member, playlist_items
                )
    ...
    def get_stream_data(self) -> dict:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if (
                self.request_type != SongRequestType.YOUTUBE_PLAYLIST
                and "&list" in self.url
            ):
...
def parse_string_query_result(result: dict) -> dict:
    ...
    thumbnails = sorted(
        result.get("thumbnails"), key=lambda x: x.get("width"), reverse=True
    )
...
def parse_playlist_response(
    original_request: str, original_member: Member, playlist_items: list[dict]
) -> list[SongRequest]:
...
def create_music_actionbar(is_paused: bool = True) -> View:
    ...
    play_button = Button(
        style=ButtonStyle.secondary, emoji="▶️", custom_id=UserActionType.PLAY.id
    )
    pause_button = Button(
        style=ButtonStyle.secondary, emoji="⏸️", custom_id=UserActionType.PAUSE.id
    )
...
class VCMusicAdmin(GroupCog, name=COG_STRINGS["music_admin_group_name"]):
    ...
    async def update_author(self):
        ...
        if new_author:
            ...
            return True
        self.logger.info(
            f"Unable to find VCMusic author with id {AUTHOR_ID}, defaulting to {MUSIC_AUTHOR}"
        )
        return False

    @command(
        name=COG_STRINGS["music_set_channel_name"],
        description=COG_STRINGS["music_set_channel_description"],
    )
    @describe(
    ...
    async def set_channel(
        ...
        embed_color: Transform[Color, ColourTransformer] = Color(0xD462FD),
        read_only: bool = True,
    ):
        ...
        await interaction.followup.send(
            content=COG_STRINGS["music_set_channel_success"].format(
...
class VCMusic(GroupCog, name=COG_STRINGS["music_group_name"]):
    ...
    async def on_voice_state_update(
        ...
        if before.channel and after.channel:
            if guild_id in self.active_players:
                self.active_players.get(guild_id).voice_client = (
                    after.channel.guild.voice_client
                )
    ...
    async def shuffle_queue_handler(self, interaction: Interaction) -> bool:
        ...
        if not self.check_valid_user(interaction.guild, interaction.user):
            await respond_or_followup(
                COG_STRINGS["music_invalid_voice"], interaction, ephemeral=True
            )
            return False

        if interaction.guild.id not in self.active_players:
            await respond_or_followup(
                COG_STRINGS["music_warn_not_playing"], interaction, ephemeral=True
            )
            return False
        ...
        await respond_or_followup(
            COG_STRINGS["music_shuffle_queue_success"], interaction, ephemeral=True
        )
    ...
    async def set_volume_submit_handler(self, interaction: Interaction) -> bool:
        ...
        for item in raw_modal_data:
            if (
                item.get("components")[0].get("custom_id")
                == UserActionType.VOLUME_MODAL_VALUE.id
            ):
        ...
        if not raw_volume_value.isdigit():
            await respond_or_followup(
                COG_STRINGS["music_volume_modal_invalid"].format(
                    supplied=raw_volume_value
                ),
                interaction,
                ephemeral=True,
            )
        ...
        self.active_players.get(interaction.guild.id).voice_client.source.volume = (
            float(volume_value) / float(100)
        )
        self.active_players.get(interaction.guild.id).volume = volume_value
        await self.update_embed(interaction.guild.id)
        await respond_or_followup(
            COG_STRINGS["music_volume_set_success"].format(value=volume_value),
            interaction,
        )
        return True
    ...
    async def add_modal_interaction_handler(self, interaction: Interaction) -> bool:
        ...
        await respond_or_followup(
            COG_STRINGS["music_thinking"], interaction, ephemeral=True, delete_after=20
        )
        ...
        for item in raw_modal_data:
            if (
                item.get("components")[0].get("custom_id")
                == UserActionType.ADD_SONG_MODAL_SINGLE.id
            ):
                single_request = item.get("components")[0].get("value")
            elif (
                item.get("components")[0].get("custom_id")
                == UserActionType.ADD_SONG_MODAL_MULTIPLE.id
            ):
        ...
        request_list = [
            ...
            for x in multiple_request.split("\n")
            if x.strip() not in ("", " ")
        ]
        ...
        if request_list:
            ...
            if song is None or song is []:
                first_request = 0
            elif await self.try_play_queue(
                interaction, add_to_queue=song if isinstance(song, list) else [song]
            ):
        ...
        await respond_or_followup(
            COG_STRINGS["music_added_song_count"].format(
                count=len(request_list) - len(failed_requests) + first_success
            ),
            interaction,
            ephemeral=True,
        )
    ...
    async def try_play_queue(
        self, interaction: Interaction, add_to_queue: list = []
    ) -> bool:
        ...
        if interaction.guild.id not in self.active_players:
            ...
            active_player = GuildMusicPlayer(
                guild=interaction.guild, voice_client=voice_client
            )
    ...
    async def resume_or_start_playback(self, interaction: Interaction) -> bool:
        ...
        if self.active_players.get(interaction.guild.id).voice_client.is_playing():
            await respond_or_followup(
                COG_STRINGS["music_warn_already_playing"], interaction, ephemeral=True
            )
            return False
        ...
        if self.active_players.get(interaction.guild.id).voice_client.is_paused():
            ...
            await respond_or_followup(
                COG_STRINGS["music_resume_success"], interaction, ephemeral=True
            )
            return True

        await respond_or_followup(
            COG_STRINGS["music_generic_error"].format(author=MUSIC_AUTHOR),
            interaction,
            ephemeral=True,
        )
        return False
    ...
    async def pause_playback(self, interaction: Interaction) -> bool:
        ...
        if voice_client.is_paused():
            await respond_or_followup(
                COG_STRINGS["music_warn_already_paused"], interaction, ephemeral=True
            )
            return False
        ...
        await respond_or_followup(
            COG_STRINGS["music_paused_success"], interaction, ephemeral=True
        )
        return True
    ...
    async def skip_song_handler(self, interaction: Interaction) -> bool:
        ...
        if self.play_next_song(interaction.guild.id):
            ...
            await respond_or_followup(
                COG_STRINGS["music_skip_success"], interaction, ephemeral=True
            )
            return True
        ...
        await respond_or_followup(
            COG_STRINGS["music_warn_no_next_song"], interaction, ephemeral=True
        )
        return False
    ...
    async def get_current_queue(self, interaction: Interaction) -> bool:
        ...
        if interaction.guild.id not in self.active_players:
            await respond_or_followup(
                COG_STRINGS["music_warn_view_queue_empty"], interaction, ephemeral=True
            )
            return True
        ...
        if len(current_queue) > 2 * QUEUE_CUTOFF + 5:
            ...
            first_set_formatted = "\n".join(
                [f"{idx+1}. {song.title}" for idx, song in enumerate(first_set)]
            )
            last_set_formatted = "\n".join(
                [
                    f"{idx+1+remaining+QUEUE_CUTOFF}. {song.title}"
                    for idx, song in enumerate(last_set)
                ]
            )
            separator = f"\n\n... and **`{remaining}`** more ... \n\n"
            ...
            formatted_queue = "\n".join(
                [f"{idx+1}. {song.title}" for idx, song in enumerate(current_queue)]
            )
        ...
        await respond_or_followup(
            queue_text, interaction, ephemeral=True, delete_after=None
        )
        return True
    ...
    async def update_embed(self, guild_id: int) -> bool:
        ...
        embed_message = (
            ...
            .get_channel(db_entry.channel_id)
            .fetch_message(db_entry.message_id)
        )

        current_embed: Embed = embed_message.embeds[0]
        if (
            self.active_players.get(guild_id)
            and self.active_players.get(guild_id).current_song
        ):
            current_song = self.active_players.get(guild_id).current_song
            volume = COG_STRINGS["music_embed_current_volume"].format(
                value=self.active_players.get(guild_id).volume
            )
            user = COG_STRINGS["music_embed_request_user"].format(
                user=current_song.request_member.mention
            )
            queue_length = COG_STRINGS["music_embed_queue_length"].format(
                length=len(self.active_players.get(guild_id).queue)
            )
            ...
            new_embed = create_music_embed(
                color=current_embed.color, author=MUSIC_AUTHOR
            )
            is_paused = True

        await embed_message.edit(
            embed=new_embed, view=create_music_actionbar(is_paused)
        )
        return True
    ...
    async def stop_playback(self, interaction: Interaction) -> bool:
        ...
        if interaction.guild.id not in self.active_players:
            ...
            await respond_or_followup(
                COG_STRINGS["music_stopped_success"], interaction, ephemeral=True
            )
            return True
    ...
    @command(
        name=COG_STRINGS["music_play_name"],
        description=COG_STRINGS["music_play_description"],
    )
    ...
    @command(
        name=COG_STRINGS["music_pause_name"],
        description=COG_STRINGS["music_pause_description"],
    )
    ...
    @command(
        name=COG_STRINGS["music_skip_name"],
        description=COG_STRINGS["music_skip_description"],
    )
    ...
    @command(
        name=COG_STRINGS["music_add_name"],
        description=COG_STRINGS["music_add_description"],
    )
    ...
    @command(
        name=COG_STRINGS["music_view_queue_name"],
        description=COG_STRINGS["music_view_queue_description"],
    )
    ...
    @command(
        name=COG_STRINGS["music_stop_name"],
        description=COG_STRINGS["music_stop_description"],
    )
    ...
    @command(
        name=COG_STRINGS["music_volume_name"],
        description=COG_STRINGS["music_volume_description"],
    )
    ...
    async def set_volume(self, interaction: Interaction, volume: Range[int, 0, 100]):
        ...
        self.active_players.get(interaction.guild.id).voice_client.source.volume = (
            float(volume) / float(100)
        )
        self.active_players.get(interaction.guild.id).volume = volume
        await self.update_embed(interaction.guild.id)
        await respond_or_followup(
            COG_STRINGS["music_volume_set_success"].format(value=volume), interaction
        )
        return True

    @command(
        name=COG_STRINGS["music_shuffle_name"],
        description=COG_STRINGS["music_shuffle_description"],
    )
